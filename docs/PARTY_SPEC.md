...
- `clear()` — clears the entire VGA text screen (80x25) to spaces
  with the current default attribute, and resets the cursor to
  `(0, 0)`. Statement only — returns no value. Equivalent to the
...
## 16. PC speaker builtins

Like every other builtin (sections 9, 10, 13, 14, 15), these are
ordinary function-shaped builtin names — a script cannot declare its
own `func` with these names; the builtin is always what runs.

```
beep(440, 250)          // 440 Hz for 250 ms on the PC speaker
beep(0, 500)            // hz=0 just silences for 500 ms (a plain wait)
play("intro.ss")        // play a Shelly Sound text script
play("intro.ssb")       // or a compiled .ssb binary - auto-detected
```

- `beep(hz, ms)` — sounds the PC speaker at `hz` Hz for `ms`
  milliseconds, then silences it — the same primitives behind the
  shell's `beep` / `sound beep` commands (`pcspk_on`, `pcspk_off`,
  `sleep_ms`). Both arguments must be non-negative ints; a non-int or
  negative argument, or a wrong argument count, is a runtime error.
  `hz=0` just silences for `ms`, equivalent to `sleep(ms)`. Statement
  only, like `fwrite`/`arr_set` — returns no value. **Esc** aborts
  the wait early, the same as it does for `sleep()` (section 14).
- `play(path)` — plays a Shelly Sound file on the PC speaker: a `.ss`
  text script, or a compiled `.ssb` binary. The two are distinguished
  by the `.ssb` magic dword `0x53534201` (spec section 6) at the
  start of the file, not by the filename, so a `.ssb` under any name
  just works. This is the
  exact same `sound_detect_and_play` path the shell's `play` /
  `sound play` commands use, and the player state is reset to spec
  defaults for the file first, so every `play()` starts fresh. `.ssb`
  binaries are produced outside scripts by the shell's
  `sound compile <file.ss>` command. `path` must be a string; a path
  that doesn't resolve to a file and a non-string argument are both
  runtime errors, as is a file too large to stage. Statement only.
  **Esc** stops the playback — and because Esc sets the script's kill
  flag, the script aborts at the next statement boundary, the same
  behavior `sleep()`'s early abort sets up everywhere else.

### 16.1 Background music — `music`, `music_step`, `music_stop`

`play()`/`beep()` block the script for the whole duration of the
sound, so they can't back a scrolling gameplay loop: the game would
freeze each time a sound started. These three builtins are the
non-blocking alternative — background music. The song is a `.ssb`
binary; it plays from a private stage buffer (not the shared file
I/O buffer, so unrelated `fread`/`play`/`rush` traffic can't corrupt
it) on the PC speaker, looping forever, while the script keeps
running.

```
music("snake_bg.ssb")   // start the looping background music (returns at once)
while (true) {
    ...move the hero...
    music_step()          // advance the song by real time since the last call
    beep(440, 50)         // a foreground sound STOPS the music (beep/play do)
    music("snake_bg.ssb") // ...so restart it if it should keep going
}
music_stop()              // end of the level, or whenever the tune should end
```

- `music(path)` — loads the `.ssb` at `path` and starts it looping on
  the speaker, then returns immediately. Replaces any previous
  background music; a `music()` call also counts as the script
  borrowing the speaker, so `beep()`/`play()` — and the shell's
  `play`/`sound play`/`sound beep`/`beep` commands — stop the music as
  their first act. `path` must be a string; a path that doesn't
  resolve to a file, a file that isn't a `.ssb`, and a non-string
  argument are runtime errors. Statement only, like `play()`.
- `music_step()` — advances the background music by the wall-clock
  time elapsed since the last `music_step()`/`music()` call (the same
  calibrated TSC clock `sleep()` uses), sounding whatever note or rest
  is now current, and returns immediately — it never blocks. It only
  ever *advances* what's playing; it does not start or stop anything.
  It is a cheap, always-safe no-op when no music is active, so a game
  loop should call it once per frame (the frame's `sleep()` is the
  cadence) and not bother checking whether music is on. If a frame
  ran late, the player catches up by skipping whole notes, so the song
  stays anchored to real time rather than to frame count. Statement
  only.
- `music_stop()` — stops the background music and silences the
  speaker. Safe to call when nothing is playing. Statement only. The
  interpreter also stops music automatically when a script finishes,
  is Esc-killed, or a fresh script starts, so a runaway loop can't
  leave a stuck tone behind.

Restarting the same (or a different) tune is just another
`music("...")` call — combined with `play()` for a one-shot sound
effect (munch, explosion, fanfare) this is how a game does "music
under a hit-stop": `play("hit.ssb")` fires the SFX and silences the
track, then `music("bg.ssb")` brings the loop right back.

`play()` takes a path, not the sound itself: a `.ss`/`.ssb` referenced
by a script is a **runtime asset**, read from the filesystem when the
call runs. This holds identically whether the script is interpreted
(`party foo.pa`) or compiled to a `.run` — the compiled binary embeds
only the source text, and the sound file still has to be on disk (in
the current directory, or at the path given) when the `.run` plays it.
`play()` accepts any string expression (`play("dir/" + fname)` works),
so the target file can't even always be known at compile time. To ship
a self-contained `.run`, put the referenced `.ss`/`.ssb` files
alongside it, just like any other data file a script reads.

```
while (true) {
    play("loop.ssb")     // shell-compiled theme song
    sleep(50)
}
```
...
`sound.pa`
```
beep(880, 60)            // a short high tick
beep(0, 60)              // a silent gap
beep(440, 60)            // and a lower one
play("theme.ss")         // then a .ss song from the current folder
play("theme.ssb")        // the compiled equivalent plays identically
```
