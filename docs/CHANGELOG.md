...
- **`sin get` / package install corrupted downloaded files** — the AHCI
  PRDT DBA was the caller's raw buffer, and every file/IO buffer sits at
  a misaligned (odd) address; this controller silently dropped the last
  byte of each 512-byte transfer on a misaligned DBA, so the on-disk
  file was truncated at each sector boundary and the installer reported
  "not a valid stored zip file (or truncated)". `ahci_rw_common`
  (`kernel.asm`) now stages unaligned DMA through a dedicated
  512-aligned `ahci_dma_bounce` buffer before issuing the PRDT (copying
  in on write, out on read), so every DBA is sector-aligned and all 512
  bytes land. `matrix2` now downloads, writes back, reads back, and
  verifies with CRC `d7602d2a` across the whole install.

### Changed

- Banner and build stamp bumped to `v0.1.23` (`kernel.asm`); the build
  stamp now reads "install fix (AHCI DMA bounce)". The sin.asm sector
  probe/diagnostic block (and its sentinel writes that clobbered file
  sectors 8/11 on disk) was removed now that byte 511 is proven to land.
