...
; body only gets EDIT_BODY_ROWS rows (see below - one short of the
; physical remainder on purpose). Must match the number of newlines
; msg_edit_header1+msg_edit_header2 print before file content.
EDIT_HEADER_ROWS equ 2

; The body must never be allowed to completely fill the screen's last
; physical row (row VGA_ROWS-1): putchar has no idea it's being driven by
; a fixed-viewport editor, so finishing that row (via an 80-col wrap or a
; real newline) makes it advance the cursor past the bottom exactly like
; a scrolling terminal - triggering scroll_screen mid-render. That shifts
; everything .ce_render already drew (including the header just printed
; moments earlier in this same call) up and off-screen, while the
; render's own row bookkeeping (r12/r9) has no idea it happened - the
; next call's cursor-position capture ends up stale/misplaced. Reserving
; one unused row at the bottom keeps every row .ce_render draws strictly
; above VGA_ROWS-1, so that transition can never happen.
EDIT_BODY_ROWS  equ VGA_ROWS - EDIT_HEADER_ROWS - 1
...
EDIT_MAX        equ 20480           ; max bytes one file can hold in the staging buffers
                                    ; (fs_io_buf / script buffers) - a cap on interactive
                                    ; view/edit/run, not on the on-disk format anymore.
                                    ; REVERTED from 40960 (2026-08-14): that build pushed
                                    ; kernel.bin's end address past rsp=0x9F000 (set in
                                    ; kernel_entry), colliding the stack with fs_io_buf/
                                    ; rr_script_buf and corrupting the editor's own view
                                    ; (blank content, garbage glyphs). Don't just bump this
                                    ; equ again without first shrinking the ~61KB it costs
                                    ; at 40960 - see the split-buffer plan in kernel_entry's
                                    ; area / ask before re-raising.
...
.ce_auth_ok:
    ...
    ; Refuse files that can't fit the staging buffer: editing a truncated
    ; copy and saving would silently destroy everything past EDIT_MAX. r15
    ; still holds the node index (popped by the save/discard paths).
    mov eax, [node_bin_len + r15*4]
    cmp eax, EDIT_MAX - 1
    jbe .ce_load_ok
    mov rsi, msg_edit_toobig
    mov al, ATTR_ERROR
    call print_string_attr
    mov rsi, arg1_buf
    call print_string
    mov rsi, msg_edit_toobig_sz
    call print_string
    mov eax, [node_bin_len + r15*4]
    lea rdi, [show_num_buf]
    call int_to_str
    ...
    mov rsi, show_num_buf
    call print_string
    mov rsi, msg_edit_toobig_tail
    call print_string
    pop r15
    ret
.ce_load_ok:
    mov rax, r15                      ; fs_read_file needs the node index here -
                                       ; rax was left holding the file's byte
                                       ; length by the size check above, not r15
    lea rdi, [fs_io_buf]
    call fs_read_file

    mov rax, r15
    call fs_file_len                 ; exact recorded length - not NUL-scan-
    cmp rax, EDIT_MAX - 1            ; derived, so an embedded NUL (or a file
    jbe .ce_len_ok                   ; that starts with one) can't make the
    mov rax, EDIT_MAX - 1            ; content appear empty/truncated. Clamp
.ce_len_ok:                          ; to match fs_read_file's own EDIT_MAX-1
                                      ; copy limit (already guaranteed not to
                                      ; be hit here since .ce_load_ok only
                                      ; runs when node_bin_len <= EDIT_MAX-1).
    push r12
    push r13
    push r14
...
.ce_delete:
    ...
    call .ce_render
    jmp .ce_loop
; .ce_up / .ce_down: move the cursor to (approximately) the same column on
; the row above/below, in the wrap-aware row sense .ce_row_of/.ce_offset_of_row
; already use for paging (a "row" is a stretch between newlines/80-col
; wraps, not a logical line). Column is clamped to the target row's length,
; excluding a trailing real newline from the valid columns (matching
; .ce_home/.ce_end, which stop cursor movement AT a newline, never past it).
.ce_up:
    mov rcx, r13
    call .ce_row_of                  ; rax = current row
    test rax, rax
    jz .ce_loop                      ; already on the first row - nothing above
    mov r11, rax                     ; r11 = current row
    mov rcx, r11
    call .ce_offset_of_row           ; rax = current row's start offset
    mov rdx, rax
    mov rdi, r13
    sub rdi, rdx                     ; rdi = column within current row
    dec r11                          ; r11 = target row (current - 1)
    jmp .ce_vert_move
.ce_down:
    mov rcx, r13
    call .ce_row_of                  ; rax = current row
    mov r11, rax
    mov rcx, r14
    call .ce_row_of                  ; rax = row the end-of-buffer falls on (last row)
    cmp r11, rax
    jae .ce_loop                     ; already on (or past) the last row - nothing below
    mov rcx, r11
    call .ce_offset_of_row           ; rax = current row's start offset
    mov rdx, rax
    mov rdi, r13
    sub rdi, rdx                     ; rdi = column within current row
    inc r11                          ; r11 = target row (current + 1)
.ce_vert_move:
    mov rcx, r11
    call .ce_offset_of_row           ; rax = target row's start offset
    mov r8, rax                      ; r8 = target row start
    mov rcx, r11
    inc rcx
    call .ce_offset_of_row           ; rax = start of the row after target (or r14)
    mov r10, rax
    mov rbx, r10
    sub rbx, r8                      ; rbx = target row's raw length
    cmp r10, r14
    jae .ce_vm_have_len              ; target row runs to end of buffer - no newline to exclude
    lea rsi, [fs_io_buf]
    cmp byte [rsi + r10 - 1], 10     ; does target row end in a real newline?
    jne .ce_vm_have_len
    dec rbx                          ; exclude it - cursor stops at the newline, not past it
.ce_vm_have_len:
    cmp rdi, rbx
    jbe .ce_vm_col_ok
    mov rdi, rbx                     ; clamp column to target row's length
.ce_vm_col_ok:
    mov r13, r8
    add r13, rdi
    call .ce_render
...
.ce_save:
    ...
    mov rax, r15
    lea rsi, [fs_io_buf]
    mov rcx, r14                     ; exact byte count in the buffer - avoids
    call fs_write_binary_file        ; truncating at an embedded NUL the way
                                      ; fs_write_file's NUL-scan write would
    call maybe_auto_sync
    call clear_screen
    mov rsi, msg_saved
    mov al, [cur_normal_attr]
...
.ce_render:
    ...
    jl .cer_scroll_up
    mov rbx, r12
    add rbx, (EDIT_BODY_ROWS - 1)
    cmp r11, rbx
    jle .cer_have_top
    mov r12, r11
    sub r12, (EDIT_BODY_ROWS - 1)
    jmp .cer_have_top
.cer_scroll_up:
    mov r12, r11
.cer_have_top:
...
.have_arg:
    ...
    cmp rax, -1
    je .not_found
    ; preview notice for files bigger than the staging buffer
    mov r12, rax
    mov eax, [node_bin_len + r12*4]
    cmp eax, EDIT_MAX - 1
    jbe .cv_read
    mov rsi, msg_view_trunc
    mov al, [cur_normal_attr]
    call print_string_attr
    mov eax, [node_bin_len + r12*4]
    lea rdi, [show_num_buf]
...
.have_arg:
    ...
    mov rax, r12
    mov eax, [node_bin_len + r12*4]
    mov r13, rax                 ; true file size - no staging buffer involved
...
.have_arg:
    ...
    cmp eax, EDIT_MAX - 1
    jbe .rr_size_ok
    mov rsi, msg_run_toobig1
    mov al, ATTR_ERROR
    call print_string_attr
    mov rsi, arg1_buf
    call print_string
    mov rsi, msg_run_toobig2
    call print_string
    mov eax, [node_bin_len + rbx*4]
    lea rdi, [show_num_buf]
    call int_to_str
...
; file's bytes from its cluster run into the destination and NUL-terminates
; them. Copying is clamped to EDIT_MAX-1 bytes: every staging destination
; (fs_io_buf / rr_script_buf halves / http_rx_buf) is EDIT_MAX-sized, so an
; oversized file can never overflow its buffer. Callers detect the clamp by
; comparing node_bin_len > EDIT_MAX-1.
fs_read_file:
    push rbx
    push rcx
    push rdx
...
.ext_up:
    ...
    jne .return_up                ; edit mode: hand the sentinel to cmd_edit so it can
                                   ; move the cursor up a line itself - terminal
                                   ; scrollback makes no sense while the editor owns the screen
...
.ext_down:
    cmp byte [edit_active], 0
    jne .return_down              ; edit mode: same as .ext_up above
    cmp byte [ctrl_state], 0
    je .return_down
    cmp byte [browse_active], 0
    jne .return_down
...
msg_take_noclen:    db "take: no Content-Length in reply.", 10, 0
...
msg_edit_toobig_tail: db " bytes). File left unchanged; use 'view' for a preview.", 10, 0
msg_view_trunc:     db "view: file is ", 0
msg_view_trunc_tail: db " bytes; showing first ", 0
msg_view_trunc_done: db " bytes.", 10, 0
msg_run_toobig1:    db "run: '", 0
msg_run_toobig2:    db "' is ", 0
msg_run_toobig3:    db " bytes - too large for the runner (max ", 0
msg_run_toobig4:    db " bytes). Script not run.", 10, 0
...
ts_hdr_len:     dd 0                 ; bytes accumulated in http_rx_buf so far
ts_hdr_end:     dd 0                 ; index of CRLFCRLF within http_rx_buf
ts_scan:        dd 0                 ; resume index for the CRLFCRLF search
ts_cl:          dd 0                 ; parsed Content-Length
ts_first:       dd 0                 ; first cluster of the allocated run
ts_run_lba:     dd 0                 ; next absolute LBA to write
ts_off:         dd 0                 ; body bytes written (node_bin_len)
ts_progress:    dd 0                 ; total bytes consumed (idle-check gauge)
ts_last_prog:   dd 0
ts_pend_len:    dd 0                 ; bytes held in fs_io_scratch partial sector
ts_hdr_done:    db 0
ts_fail:        db 0
